...
def download(url, directory_path='current'):
    ...
    print(f'directorypath_files = {directorypath_files}')
    ...
    for href in links:
        if is_local(href):  # checking if it is local
            list_of_links.append(href.get('href'))  # get image sources

    list_of_scripts = []
    scripts = file_contents.find_all('script')  # find all the scripts: with and without 'src'
    for source_ in scripts:
        if is_local(source_) and source_['src']:
            list_of_scripts.append(source_.get('src'))  # get image sources
